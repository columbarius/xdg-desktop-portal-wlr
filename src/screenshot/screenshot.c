...
static pid_t spawn_chooser(int chooser_out[2]) {
	pid_t pid = fork();
	if (pid < 0) {
		perror("fork");
	} else if (pid == 0) {
...
static char *exec_coordinates_selector() {
	...
	pid_t pid = spawn_chooser(chooser_out);
	if (pid < 0) {
...
static int method_screenshot(sd_bus_message *msg, void *data,
	...
	logprint(INFO, "dbus: handle: %s", handle);
	...
	if (interactive) {
		...
		if (coordinates == NULL || strlen(coordinates) == 0) {
			free(coordinates);
			return -1;
		}
	}
